...
The data frame contains `r nrow(problem_2_df)*ncol(problem_2_df)` total observations, with `r nrow(problem_2_df)` rows and `r ncol(problem_2_df)` columns. The mean of `x` is `r mean(problem_2_df$x)` and the median of `x` is `r median(problem_2_df$x)`. The proportion of cases for which the logical vector `vec_logical` is TRUE is `r mean(problem_2_df$vec_logical)`.
...
# Set up scatterplot of Y vs. X
ggplot(problem_2_df, aes(x = x, y = y)) + geom_point(aes(color = vec_logical))

# Saved scatterplot to project directory
ggsave("Problem_2_Scatterplot.pdf", height = 4, width = 6)

```

A scatterplot of `y` vs `x` was created that color points using `vec_numerical`.
...
A scatterplot of `y` vs `x` was created that color points using `vec_factor`.
