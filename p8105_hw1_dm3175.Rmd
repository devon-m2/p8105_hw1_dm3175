...
The data frame contains `r dim(problem_2_df)` total observations, with `r nrow(problem_2_df)` rows and `r ncol(problem_2_df)` columns. The mean of `x` is `r mean(problem_2_df$x)` and the median of `x` is `r median(problem_2_df$x)`. The proportion of cases for which the logical vector `vec_logical` is TRUE is `r mean(problem_2_df$vec_logical)`.
